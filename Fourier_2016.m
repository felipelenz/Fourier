...
% and sweep number can be found in the 2014 Summary Table (HBM trigger
% column)
...
figure;
% subplot(211)
plot(([0:length(data1)-1].*(1/fs)).*1e6,data1./max(data1)); hold on
plot(([0:length(data2)-1].*(1/fs)).*1e6,data2./max(data2)); hold on
plot([0:length(h)-1].*(1/fs).*1e6,h,'r'); hold on;
plot([0:length(h_noise)-1].*(1/fs).*1e6,h_noise,'m'); hold on;
xlabel('Time (\mus)');
...
L_phase=unwrap(angle(numerator));
L_ampl=smooth(20*log10(abs(numerator)),10);
%% Error Analysis
high_freq=4e6;
freq=[0:n-1].*(fs/n);
...
%% Measure phase
fc=1800; %in kHz
high_freq_cuttoff=round(fc*1e3/(freq(2)-freq(1)));
...
c=2.99e8;
prop_delay=293/c;
plot((IIHI.time-LC_OT_delay).*1e6,IIHI.data(:,seg)./max(IIHI.data(:,seg))); hold all
plot((D1.time-prop_delay).*1e6,D1.data(:,seg)./max(D1.data(:,seg)));
plot((D2.time-prop_delay).*1e6,D2.data(:,seg)./max(D2.data(:,seg)));
xlabel('Time (\mus)');
ylabel('Normalized Current and Luminosity');
legend('Current','Luminosity D1# (2015 APD)','Luminosity D2# (2014 APD)');
xlim([-20,50]);
grid on;
...
grid on;
subplot(313)
plot(freq./1e3,amplitude);
xlim([0,fc]);
xlabel('Frequency (kHz)');
ylabel('Amplitude (dB)');
grid on;
%%
high_freq_cuttoff=3500;
    
H_phase=unwrap(angle(transfer));
H_ampl=abs(transfer);

I_phase=unwrap(angle(numerator./transfer));
I_ampl=(abs(numerator./transfer));

I_f=I_ampl.*exp(sqrt(-1).*I_phase);
time_domain_I=real(ifft(I_f));
plot(time_domain_I);
%%
figure
subplot(121)
semilogy(freq(2:end/2),I_ampl(2:end/2));
subplot(122)
plot(freq(2:end/2),I_phase(2:end/2));